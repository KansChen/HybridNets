...
def annotate_lane_numbers(
    seg_mask: np.ndarray, seg_img: np.ndarray, lane_idx: int
) -> None:
    """Draw ordered lane numbers on the segmentation image.

    Parameters
    ----------
    seg_mask : np.ndarray
        Segmentation mask containing class indices.
    seg_img : np.ndarray
        Image to annotate (modified in-place).
    lane_idx : int
        Class index corresponding to the lane mask.
    """

    lane_mask = (seg_mask == lane_idx).astype(np.uint8)
    if lane_mask.max() == 0:
        return
    num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(lane_mask)
    lane_order = sorted(range(1, num_labels), key=lambda x: centroids[x][0])
    for order, label_id in enumerate(lane_order, start=1):
        x, y = int(centroids[label_id][0]), int(centroids[label_id][1])
        cv2.putText(
            seg_img,
            str(order),
            (x, y),
...
parser.add_argument(
    "--source", type=str, default="demo/image", help="The demo image folder"
)
...
parser.add_argument(
    "--imwrite", type=boolean_string, default=True, help="Write result to output folder"
)
parser.add_argument(
    "--show_det",
    type=boolean_string,
    default=False,
    help="Output detection result exclusively",
)
parser.add_argument(
    "--show_seg",
    type=boolean_string,
    default=False,
    help="Output segmentation result exclusively",
)
...
parser.add_argument(
    ...
    type=boolean_string,
    default=True,
    help="Use float16 for faster inference",
)
parser.add_argument(
    "--speed_test", type=boolean_string, default=False, help="Measure inference latency"
)
...
lane_idx: Optional[int] = seg_list.index("lane") + 1 if "lane" in seg_list else None

color_list = standard_to_bgr(STANDARD_COLORS)
ori_imgs = [
    cv2.imread(i, cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION) for i in img_path
]
...
for ori_img in ori_imgs:
    ...
    input_img = cv2.resize(
        ori_img, (int(w0 * r), int(h0 * r)), interpolation=cv2.INTER_AREA
    )
    h, w = input_img.shape[:2]

    (input_img, _), ratio, pad = letterbox(
...
with torch.no_grad():
    ...
    if seg_mode == BINARY_MODE:
        ...
        seg_mask_list = [
            torch.where(torch.sigmoid(seg)[:, i, ...] >= 0.5, 1, 0)
    ...
    for i in range(seg.size(0)):
        ...
        for seg_class_index, seg_mask in enumerate(seg_mask_list):
            ...
            seg_mask_ = seg_mask_[
                pad_h : seg_mask_.shape[0] - pad_h, pad_w : seg_mask_.shape[1] - pad_w
            ]
            seg_mask_ = cv2.resize(
                seg_mask_, dsize=shapes[i][0][::-1], interpolation=cv2.INTER_NEAREST
            )
            color_seg = np.zeros(
                (seg_mask_.shape[0], seg_mask_.shape[1], 3), dtype=np.uint8
            )
            ...
            color_mask = np.mean(
                color_seg, 2
            )  # (H, W, C) -> (H, W), check if any pixel is not background
            # prepare to show det on 2 different imgs
            # (with and without seg) -> (full and det_only)
            det_only_imgs.append(ori_imgs[i].copy())
            seg_img = (
                ori_imgs[i].copy() if seg_mode == MULTILABEL_MODE else ori_imgs[i]
            )  # do not work on original images if MULTILABEL_MODE
            seg_img[color_mask != 0] = (
                seg_img[color_mask != 0] * 0.5 + color_seg[color_mask != 0] * 0.5
            )
            if lane_idx is not None:
                annotate_lane_numbers(seg_mask_, seg_img, lane_idx)
            seg_img = seg_img.astype(np.uint8)
            seg_filename = (
                f"{output}/{i}_{params.seg_list[seg_class_index]}_seg.jpg"
                if seg_mode == MULTILABEL_MODE
                else f"{output}/{i}_seg.jpg"
            )
    ...
    for i in range(len(ori_imgs)):
        out[i]["rois"] = scale_coords(
            ori_imgs[i][:2], out[i]["rois"], shapes[i][0], shapes[i][1]
        )
        ...
        if show_det:
            cv2.imwrite(
                f"{output}/{i}_det.jpg",
                cv2.cvtColor(det_only_imgs[i], cv2.COLOR_RGB2BGR),
            )
        ...
        if imwrite:
            cv2.imwrite(
                f"{output}/{i}.jpg", cv2.cvtColor(ori_imgs[i], cv2.COLOR_RGB2BGR)
            )
